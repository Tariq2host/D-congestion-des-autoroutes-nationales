...
# Initialize the DAG
dag = DAG(
    'ETL_toll_data_bash_with_kafka',
    default_args=default_args,
    description='A simple ETL DAG for toll data using BashOperator and Kafka',
    schedule_interval=timedelta(days=1),
)

# Kafka producer to indicate the start of the ETL process
kafka_produce_start = BashOperator(
    task_id='kafka_produce_start',
    bash_command="""
    echo 'ETL process started at $(date)' | kafka-console-producer --broker-list localhost:9092 --topic etl_topic
    """,
    dag=dag,
)
...
# Kafka producer to indicate the end of the ETL process
kafka_produce_end = BashOperator(
    task_id='kafka_produce_end',
    bash_command="""
    echo 'ETL process completed at $(date)' | kafka-console-producer --broker-list localhost:9092 --topic etl_topic
    """,
    dag=dag,
)

# Define task pipeline
kafka_produce_start >> unzip_data >> extract_data_from_csv >> extract_data_from_tsv >> consolidate_data >> transform_data >> kafka_produce_end
